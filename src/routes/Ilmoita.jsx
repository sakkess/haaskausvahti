...
export default function Ilmoita() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      /* reset */
      setOtsikko(''); setKuvaus('');
      setCofog1(''); setCofog2(''); setCofog3('');
      ...
      console.error(err); alert(`Jokin meni pieleen: ${err.message}`);
    }
  };
  ...
  return (
    <Card>
      <h2 className="mb-6 text-h2">Tee säästöaloite</h2>

      <form onSubmit={handleSubmit} className="space-y-6">
        {/* --- all existing inputs exactly as before --- */}
        {/* ( copy‑paste the inputs from your previous version ) */}

        {/* Submit */}
        <Button type="submit" className="w-full">Lähetä säästöaloite</Button>
      </form>
    </Card>
  );
}
